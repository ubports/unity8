...
Showable {
    ...
    function sessionToStart() {
        for (var i = 0; i < LightDM.Sessions.count; i++) {
            var session = LightDM.Sessions.data(i, LightDM.SessionRoles.KeyRole);
            if (loader.item.sessionToStart === session) {
                return session;
            }
        }

        if (loader.item.sessionToStart === LightDM.Greeter.defaultSession) {
            return LightDM.Greeter.defaultSession;
        } else {
    ...
    QtObject {
        ...
        readonly property int selectUserIndex: d.getUserIndex(LightDM.Greeter.selectUser)
        ...
        function getUserIndex(username) {
            ...
            for (var i = 0; i < LightDM.Users.count; i++) {
                if (username === LightDM.Users.data(i, LightDM.UserRoles.NameRole)) {
                    return i;
                }
            }
        ...
        function selectUser(index, reset) {
            if (index < 0 || index >= LightDM.Users.count)
                return;
            d.waiting = true;
            if (reset) {
            ...
            currentIndex = index;
            var user = LightDM.Users.data(index, LightDM.UserRoles.NameRole);
        ...
        function login() {
            d.waiting = true;
            var success = LightDM.Greeter.startSessionSync(root.sessionToStart());
            d.waiting = false;
