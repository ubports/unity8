...
class ContentTimer : public LomiriIndicators::AbstractTimer
{
    Q_OBJECT
public:
    ContentTimer(QObject *parent) : LomiriIndicators::AbstractTimer(parent) {
        m_timer.setSingleShot(false);
        connect(&m_timer, &QTimer::timeout,
                this, &LomiriIndicators::AbstractTimer::timeout);
    }
    int interval() const override { return m_timer.interval(); }
    void setInterval(int msecs) override { m_timer.setInterval(msecs); }
    void start() override { m_timer.start(); LomiriIndicators::AbstractTimer::start(); }
    void stop() override { m_timer.stop(); LomiriIndicators::AbstractTimer::stop(); }
private:
    QTimer m_timer;
};
...
public:
    ...
    int m_delta;
    int m_count;
    LomiriIndicators::AbstractTimer* m_timer;
...
void MenuContentActivator::setContentTimer(LomiriIndicators::AbstractTimer *timer)
{
    int interval = 0;
    ...
    connect(timer, &LomiriIndicators::AbstractTimer::timeout,
