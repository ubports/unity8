...
void Lens::setUnityLens(const unity::dash::Lens::Ptr& lens)
{
    m_unityLens = lens;
    ...
    /* Signals forwarding */
    connect(this, SIGNAL(searchFinished(unity::dash::Lens::Hints const &, unity::glib::Error const &)), SLOT(onSearchFinished(unity::dash::Lens::Hints const &)));
...
unity::dash::Lens::Ptr Lens::unityLens() const
{
    return m_unityLens;
}
...
void Lens::onGlobalResultsChanged(const unity::dash::Results::Ptr& /* global_results */)
{
    m_globalResults->setName(QString::fromStdString(m_unityLens->global_results()->swarm_name));
}
